...
This paper extends the work presented in \cite{mirabel-iros2016} and \cite{MirLam2018} with the following new material:
\begin{itemize}
...
\mbox{if} & P(C_{o_i},f_j)\mbox{ outside } f_j\end{array}\right.
$$
$\bar{h}_2$, $\bar{h}_3$, $\bar{h}_6$ respectively represent the translation in $x-y$ plane and the rotation around $z$-axis of frame $o_i$ with respect to frame $f_j$. Let $\conf_0$ be a configuration such that $h(\conf_0)=0$. The sub-manifold defined by
