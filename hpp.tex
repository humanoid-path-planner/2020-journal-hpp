...
This \paper describes a software platform taylored for manipulation planning in
robotics.
...
\Partie~\ref{partie:preliminary} introduces some preliminary notions like kinematic chains and Lie groups that are used to model the configuration space of each joint. \Partie~\ref{sec:constraints} introduces non linear constraints and solvers that are at the core of manipulation problem definition. \Partie~\ref{sec:manipulation-problem} defines the problem of prehensile manipulation in the general setting. \Partie~\ref{sec:manipulation-planning} provides a general algorithm that solves manipulation planning problems. Finally, \partie~\ref{sec:humanoid-path-planner} is devoted to a software platform implementing the notions introduced in the previous sections.

Each \partie is implemented by one or several software packages. Instead of providing formulas for some values that need to be computed, we provide a link to the C++ implementation.
...
deformable objects~\cite{LamKav2001},\\
\cite{RouFerTai2020} and many other applications like coverage path planning~\cite{coverage2001},\\ \cite{coverage2013}, or pursuit evasion planning~\cite{pursuit-evasion1999}.

Planning motions for high dimensional robots like humanoid robots or multi-arm
system has been proven to be of a very high complexity~\cite{SchSha1983},\\ \cite{Canny1983}. Starting in the 1990's random sampling methods have been proposed
to solve the problem, trading the completeness property against efficiency in
solving problem in high dimensional configuration spaces\\ \cite{KSLO1996}, \cite{HsuLatMot99}, \cite{KufLav00}. Since then, asymptotically optimal random sampling algorithms have
been proposed~\cite{KarFra2011}.

\subsection{Path planning with non-linear constraints}

Some systems are subject to non-linear constraints. These non-linear constraints
define a sub-manifold of the configuration the robot must stay on. For instance,
legged robots that must keep contact with the ground and enforce quasi-static
equilibrium, or multi-arm systems grasping a same object are subject to this
type of constraints. As the volume of the constrained manifold is usually equal
to zero, sampling random configurations satisfying the constraints is an event
of zero probability. To sample configurations on the constrained manifold,
\cite{DalNakLamLau2009} and \cite{berenson2011} projects random configurations using a generalization of
Newton-Raphson algorithm. Another way to sample configurations on a constrained
manifold consist in expressing some configuration variables with respect to
others~\cite{Cortes2002},\\ \cite{MirLam2018} when this is possible.\\ \cite{atlasRRT} propose another method based on non-linear projection. They cover the
constrained manifold by growing an atlas composed of local charts. This
approximation provides a probability distribution that is closer to the uniform
distribution over the manifold than the projection of a uniform distribution
...
some objects are moved by some robots. The configuration space of the
whole system is constrained by non-linear constrained due to the fact
that objects cannot move by themselves and should stay in a stable
pose when not grasped by a robot.  The accessible configuration space
is thus a union of sub-manifold as defined in the previous
section. Each of these manifold is moreover a foliation where each
leaf corresponds to a stable position of an object or to a grasp of an
object by a gripper. The geometrical structure of the problem is now
well understood~\cite{AlaSimLau89}. Although several instances of
...
Rearrangement planning \cite{ota2004rearrangement}, \cite{LIS210}, consists in finding a sequence of manipulation paths that
move some objects from an initial pose to a final pose. The final configuration
of the robot is not specified. A simplifying asumption is the existence of a
monotone solution, that is a solution where each object is grasped once only and
is moved from its initial pose to its final pose \cite{stilman2008planning}, \cite{stilman2007manipulation}, \cite{srivastavaetal2014icra}, \cite{nieuwenhuisen2008effective}, \cite{ota2004rearrangement}. They mostly use two-level methods
composed of a symbolic or task planner and of a motion planner~\cite{lavalle2006planning}.
...
\begin{algorithm}
  \begin{algorithmic}[1]
    ...
    \EndIf
    \If {$p$ collision free} {\Return $p$}
    \Else { \Return collision-free portion of $p$ starting at $\conf_{near}$}
    \EndIf
    \EndFunction
    ...
    \EndFunction
  \end{algorithmic}
  \caption{Manipulation RRT algorithm iteratively calls method {\scriptsize ONE}{\small S}{\scriptsize TEP} until a solution path is found or the maximum number of iterations is reached.}
  \label{algo:M-RRT}
\end{algorithm}
...
\subsection{Waypoint transitions}
...
In case 2, we replace the transitions between $\state_1$ and $\state_2$ by three additional waypoint states and eight waypoint transitions as explain in Figure~\ref{fig:pregrasp-placement}.
...
that lie in the intermediate waypoint states, each waypoint is obtained by
...
Figure~\ref{fig:MRRT-extension} top. Table~\ref{table:waypoints} shows the
...
\begin{tabular} {|l|l|}
  \hline
  $\conf_{near}$ &
  \begin{tabular}{l} \textbf{state} $(\emptyset,\emptyset)$ \\
  \end{tabular}\\
  \hline
  $\conf_1$ &
  ...
  \hline
  $\conf_{target}$ &
  \begin{tabular}{l} \textbf{state} $(1,\emptyset)$ \\
  ...
  \hline
\end{tabular}
\caption{Waypoint configurations computed along edge $(\emptyset,\emptyset)$.
...
Note that waypoint states are internal to a waypoint edges and are not known by
the constraint graph when determining in which state a configuration lies
...
examples described in the next sections. After installing docker on a Linux machine, type

\noindent
{\small\texttt{xhost +\\docker run --rm -it \\ --name hpp --volume="/tmp/.X11-unix:/tmp/.X11-unix"\\ gitlab.laas.fr:4567/humanoid-path-planner/hpp-doc/\\ journal2020}}
